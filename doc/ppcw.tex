...
The third phase has two main goals.
The first goal is making code more readable, e.g.\
writing idiomatic Fortran code, array syntax instead of loops,
removing unnecessary variables and computation and fusing loops by
hand.
This rewrite was necessary not only for increasing maintainability,
but also to make the second goal easier to achieve.
The second goal was about hardware and environment specific
optimizations, mainly enabling vectorization.
As will be shown below, idiomatic and modern Fortran code makes it
easy for the compiler to auto-vectorize and only a small portion of
the rewrite was actually about enforcing vectorization, where the
compiler could not auto-vectorize.
...
program was increased maintainability and elegance rather than pure
speed.
...
Algorithm~\ref{alg:old} shows how the update operation per time step,
as it was originally implemented.
While, as stated above, computing $\vec{F}_i$ without the pairwise
forces (Equation~\ref{eq:F_all} without
$\sum_{i \neq j}^n f_{i \leftarrow j}$) is not the critical section,
the code is still badly written and not optimal.
...
a single loop setting $\vec{F}_i := -vis_i(\vec{v}_i + \vec{w}) +
  \vec{F}_{i \leftarrow central}$.
Fusing the loops together by hand reduced in a regression concerning
...
Unfortunately, making this change results in a performance regression
of about 1\% to the version with the fused loops.
...
$\vec{F}_i$ were exchanged to being particle-wise.
While now not leveraging the locality, removing $r$ actually reduces
...
cache.
When doing particle-wise operations, what actually happens to the
loop nest is that the inner loop over the three dimensions gets
