...
The last part of the rewriting process was to increase the cache
...
because of possible alignment issues with the transposed version.
Vectorization works best with an aligned base pointer.
...
Intel's Fortran compiler version 18.0.5 used so far automatically
aligns all statically and dynamically allocated variables and arrays.
...
% some observations made during the performance optimization:

% idiomatic code -> fast code (enables compiler to do most)

% compiler is the one making the performance increase significantly

% human should only write code to make it easy for the compiler to do
% its magic
